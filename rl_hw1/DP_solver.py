...
# # RTDP
class AsyncDynamicProgramming_RTDP(DynamicProgramming):
    def __init__(self, grid_world: GridWorld, discount_factor: float = 1.0):
        super().__init__(grid_world, discount_factor)
        self.values = np.ones(grid_world.get_state_space())
        # Initialize legal_actions for each state with all possible actions
        self.legal_actions = []
        for state in range(grid_world.get_state_space()):
            self.legal_actions.append(list(range(grid_world.get_action_space())))
        # Initialize unsolved states - all states start as unsolved
        self.unsolved = set(range(grid_world.get_state_space()))
    ...
    def _is_terminal(self, state: int) -> bool:
        _, _, done = self.grid_world.step(state, 0)
        return done

    def _greedy_action(self, state: int) -> int:
        q_values = []
        for a in self.legal_actions[state]:
            q_values.append(self.get_q_value(state, a))
        best_action_idx = np.argmax(q_values)
        return self.legal_actions[state][best_action_idx]
    
    def Real_time_DP(self) -> float:
        # Only choose start_point from unsolved states

        unsolved_list = list(self.unsolved)
        state_permutation = np.random.permutation(unsolved_list)
        delta = 0.0
        for i in range(len(state_permutation)):
            state = state_permutation[i]
            if self._is_terminal(state):
                continue

            
            max_steps = int(self.grid_world.get_state_space() * 4)
            steps = 0

            while steps < max_steps:
                new_v = self.get_state_value(state)
                diff = abs(new_v - self.values[state])
                if diff > delta:
                    delta = diff
                self.values[state] = new_v
                
                # Check if state is solved (residual < threshold)
                if diff < self.threshold and state in self.unsolved:
                    self.unsolved.remove(state)

                a = self._greedy_action(state)
                # Update policy along the trajectory
                self.policy[state] = a
                
                next_state, _, done = self.grid_world.step(state, a)
                if done:
                    break

                # If action hits wall (next_state == state), remove it from legal_actions
                if next_state == state:
                    if a in self.legal_actions[state]:
                        self.legal_actions[state].remove(a)
                    else:
    ...
    def run(self) -> None:
        delta = 2*self.threshold
        while delta > self.threshold:
            delta = self.Real_time_DP()

# Prioritized Sweeping
class AsyncDynamicProgramming_PrioritizedSweeping(DynamicProgramming):
    def __init__(self, grid_world: GridWorld, discount_factor: float = 1.0):
        """Constructor for ValueIteration
...
# In-place_DP
class AsyncDynamicProgramming_Inplace(DynamicProgramming):
    def __init__(self, grid_world: GridWorld, discount_factor: float = 1.0):
        """Constructor for ValueIteration
