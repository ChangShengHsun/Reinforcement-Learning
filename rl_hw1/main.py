...
from DP_solver import (
    ...
    AsyncDynamicProgramming_RTDP,
    AsyncDynamicProgramming_PrioritizedSweeping,
    AsyncDynamicProgramming_Inplace,
)
...
def run_async_dynamic_programming(grid_world: GridWorld):
    print(bold(underline("Async Dynamic Programming: Optimized Prioritized Sweeping")))
...
def run_async_dynamic_programming_inplace(grid_world: GridWorld):
    print(bold(underline("Async Dynamic Programming: In-place DP")))
    async_dynamic_programming_inplace = AsyncDynamicProgramming_Inplace(
        grid_world, discount_factor=DISCOUNT_FACTOR
    )
    async_dynamic_programming_inplace.run()
    grid_world.visualize(
        async_dynamic_programming_inplace.values,
        async_dynamic_programming_inplace.policy,
        title=f"Async Dynamic Programming In-place",
        show=False,
        filename=f"async_dynamic_programming_inplace.png",
    )
    print(f"Solved in {bold(green(grid_world.get_step_count()))} steps")
    history = grid_world.run_policy(async_dynamic_programming_inplace.policy, 0)
    print(
        f"Start state: {bold(green(history[0][0]))}, End state: {bold(red(history[-1][0]))}"
    )
    grid_world.reset()
...
def run_async_dynamic_programming_prioritizedsweeping(grid_world: GridWorld):
    print(bold(underline("Async Dynamic Programming: Prioritized Sweeping")))
    async_dynamic_programming_ps = AsyncDynamicProgramming_PrioritizedSweeping(
        grid_world, discount_factor=DISCOUNT_FACTOR
    )
    async_dynamic_programming_ps.run()
    grid_world.visualize(
        async_dynamic_programming_ps.values,
        async_dynamic_programming_ps.policy,
        title=f"Async Dynamic Programming Prioritized Sweeping",
        show=False,
        filename=f"async_dynamic_programming_prioritizedsweeping.png",
    )
    print(f"Solved in {bold(green(grid_world.get_step_count()))} steps")
    history = grid_world.run_policy(async_dynamic_programming_ps.policy, 0)
    print(
        f"Start state: {bold(green(history[0][0]))}, End state: {bold(red(history[-1][0]))}"
    )
    grid_world.reset()
...
def run_async_dynamic_programming_rtdp(grid_world: GridWorld):
    print(bold(underline("Async Dynamic Programming: RTDP")))
    async_dynamic_programming_rtdp = AsyncDynamicProgramming_RTDP(
        grid_world, discount_factor=DISCOUNT_FACTOR
    )
    async_dynamic_programming_rtdp.run()
    grid_world.visualize(
        async_dynamic_programming_rtdp.values,
        async_dynamic_programming_rtdp.policy,
        title=f"Async Dynamic Programming RTDP",
        show=False,
        filename=f"async_dynamic_programming_rtdp.png",
    )
    print(f"Solved in {bold(green(grid_world.get_step_count()))} steps")
    history = grid_world.run_policy(async_dynamic_programming_rtdp.policy, 0)
    print(
        f"Start state: {bold(green(history[0][0]))}, End state: {bold(red(history[-1][0]))}"
    )
    grid_world.reset()
...
if __name__ == "__main__":
    ...
    run_async_dynamic_programming_inplace(grid_world)
    run_async_dynamic_programming_prioritizedsweeping(grid_world)
    run_async_dynamic_programming_rtdp(grid_world)
